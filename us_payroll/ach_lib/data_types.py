	...
	def validate_numeric_field(self, field, length):
		...
		if field.isdigit():
			if len(field) < length:
				...
				raise AchError(f"field can only be {length} digits long")
		else:
...
class Header(Ach):
	...
	def __setattr__(self, name, value):
		if name in self.alpha_numeric_fields:
			...
			raise AchError(f"{name} not in alpha numeric field list")
...
class FileControl(Ach):
	...
	def __init__(self, batch_count, block_count, entadd_count, entry_hash, debit_amount, credit_amount):
		"""
	...
	def __setattr__(self, name, value):
		if name in self.numeric_fields:
			...
			raise AchError(f"{name} not in numeric field list")
	...
	def __setattr__(self, name, value):
		if name in self.numeric_fields:
			if name == "serv_cls_code" and str(value) not in self.serv_cls_code_list:
				raise AchError(f"{value} not in serv_cls_code_list")

			value = self.validate_numeric_field(value, self.field_lengths[name])

		elif name in self.alpha_numeric_fields:
			if name == "std_ent_cls_code" and str(value) not in self.std_ent_cls_code_list:
				raise AchError(f"{value} not in std_ent_cls_code_list")

			value = self.validate_alpha_numeric_field(value, self.field_lengths[name])

		else:
			raise AchError(f"{name} not in numeric or alpha numeric fields list")
...
class BatchControl(Ach):
	...
	def __setattr__(self, name, value):
		if name in self.numeric_fields:
			...
			raise AchError(f"{name} not in numeric_fields or alpha_numeric_fields")
...
class EntryDetail(Ach):
	...
	def get_row(self):
		...
		if self.std_ent_cls_code in ["ARC", "BOC"]:
			...
			ret_string += (
				self.id_number + self.num_add_recs + self.recv_cmpy_name + self.reserved + self.disc_data
			)
			...
			ret_string += (
				self.card_exp_date + self.doc_ref_num + self.ind_card_acct_num + self.card_tr_typ_code_shr
			)
	...
	def calc_check_digit(self):
		...
		for num, mult in zip(list(self.recv_dfi_id), multipliers, strict=False):
...
class AddendaRecord(Ach):
	...
	def __setattr__(self, name, value):
		if name in self.alpha_numeric_fields:
			...
			raise AchError(f"{value} not in numeric or alpha numeric fields")
