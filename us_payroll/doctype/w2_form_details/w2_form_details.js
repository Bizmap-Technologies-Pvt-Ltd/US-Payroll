...
frappe.ui.form.on("W2 Form Details", {
	...
	employee: function(frm) {
		if (frm.doc.employee) {
			frappe.db.get_value('Employee', frm.doc.employee, ['name', 'first_name', 'last_name',  'custom_nomasked_social_security_number', 'employee_name','custom_address_line_1', 'custom_address_line_2', 'custom_city', 'custom_stateprovince', 'custom_zip_postal_code'], function(data) {
				if (data) {
					let full_address = `${data.employee_name} \n${data.custom_address_line_1 || ''} \n${data.custom_address_line_2 || ''}\n${data.custom_city || ''}  ${data.custom_stateprovince || ''} ${data.custom_zip_postal_code || ''}`;
					frm.set_value('control_number', data.name);
					frm.set_value('first_name', data.first_name);
					frm.set_value('last_name', data.last_name);
					frm.set_value('social_security_number', data.custom_nomasked_social_security_number);
