frappe.ui.form.on("Salary Structure Assignment", {
	setup: function (frm) {
		...
		frm.set_query(
			"salary_component",
			"custom_employee_insurance_deduction",
					...
					if (r.message) {
						...
						if (salary_structure.earnings) {
							salary_structure.earnings.forEach((earning) => {
								...
								row.is_recurring_additional_salary =
									earning.is_recurring_additional_salary;
						...
						if (salary_structure.deductions) {
							salary_structure.deductions.forEach((deduction) => {
								...
								row.is_recurring_additional_salary =
									deduction.is_recurring_additional_salary;
...
frappe.ui.form.on("Employee Insurance Deduction", {
	// child doctype
		...
		frappe.db
			...
			.then((r) => {
				...
				Object.keys(field_map).forEach((sc_field) => {
					frappe.model.set_value(cdt, cdn, field_map[sc_field], data[sc_field]);
				});
			});
	},
