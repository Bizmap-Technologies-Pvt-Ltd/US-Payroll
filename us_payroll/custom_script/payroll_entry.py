...
def calculate_holiday_hours(doc):
	"""
	Calculates total holiday hours based on holidays within the given date range
	(excluding weekends) and updates custom holiday hours & amount for employees.
	"""
...
def calculate_employee_totals(doc, start_date, end_date):
	...
	for employee_row in payroll_entry.get('employees', []):
		...
		if employee_id:
		# Query Attendance records for the employee within the date range
