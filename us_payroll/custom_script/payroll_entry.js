...
frappe.ui.form.on("Payroll Entry", {
	refresh: function (frm) {
		...
		if (frm.doc.docstatus === 1 && !frm.doc.salary_slips_submitted) {
			frm.add_custom_button(__("Submit Salary"), () =>
	...
	set_total_amount: function (frm) {
		if (frm.doc.employees.length > 0 && frm.doc.docstatus == 0) {
			frm.add_custom_button(__("Set Total Amount"), function () {
			...
			callback: function (r) {
				if (r.message) {
					...
					if (bank_entry_jv && bank_entry_jv.length > 0 && check_entry_jv.length == 0) {
						frm.add_custom_button(__("Make Check Entry"), function () {
							check_entry(frm);
						}).addClass("btn-primary");
					} else {
	...
	change_button_label: function (frm) {
		$(".page-head-content")
			.find('[data-label="Submit%20Salary%20Slip"]')
			.text("Submit Salary");
	},
				...
				frm.doc.employees.forEach(function (employee) {
					...
					if (employee_id && employee_totals[employee_id]) {
						frappe.model.set_value(
							...
							employee.name,
							"custom_total_amount",
							employee_totals[employee_id]["custom_total_amount"]
						);
						frappe.model.set_value(
							employee.doctype,
							employee.name,
							"custom_total_overtime_amount",
								...
								frappe.call({
									...
									callback: function (r) {
										if (r.message && r.message.length > 0) {
											...
											frappe.msgprint(
												__(
													"No bank entry found. ACH process cannot proceed."
												)
											);
										}
									},
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (r.message && r.message.updated_slip) {
						...
						frappe.msgprint(
							`New check no <b>${r.message.new_check_no}</b> assigned to the employee <b>${r.message.emp_name}</b>`
						);
					}
					...
					frappe.db
						...
						.then((default_hours) => {
							...
							if (
								...
								row.custom_total_overtime_hours =
									row.custom_total_working_hours - max_working_hours;
								row.custom_total_working_hours = max_working_hours;
							} else {
