...
frappe.ui.form.on("Leave Allocation Tool", {
    ...
    set_fiscal_year_dates: function(frm) {   
        if (!frm.doc.from_date) frm.set_value("from_date", erpnext.utils.get_fiscal_year(frappe.datetime.get_today(), true)[1],);
        if (!frm.doc.to_date) frm.set_value("to_date", erpnext.utils.get_fiscal_year(frappe.datetime.get_today(), true)[2]);       
    },
            ...
            callback: function(r) {
                ...
                if (r.message.created && r.message.created.length > 0) {
                    r.message.created.forEach(c => {
                        messages.push(`${c.leave_type} Leave Allocation record created for the employee ${c.employee}.`);
                    });
                }

                if (r.message.skipped && r.message.skipped.length > 0) {
                    r.message.skipped.forEach(s => {
                        messages.push(`${s.leave_type} Leave Allocation record updated for the employee ${s.employee}.`);
                    });
                }
