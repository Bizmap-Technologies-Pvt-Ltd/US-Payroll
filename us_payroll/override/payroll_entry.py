...
# 		return account
	...
	# 		print(provider.as_dict().keys(), "+++++++++++++++++")


	# 		filters = {"parent": salary_component, "custom_fund": fund}
	# 		print("before forrrrrrrrrrrrrrr 111111")

	# 		# account = None
	# 		for acc_row in provider.accounts:
	# 			print("after forrrrrrrrrrrrrrr 22222222222222")
	# 			# if acc_row.category != row.category:
	# 			# 	continue

	# 			if acc_row.custom_fund != fund:
	# 				print(acc_row.custom_fund,fund, "ifffffffffffffff 111111")
	# 				continue

	# 			if department and acc_row.custom_department_name != department:
	# 				print(acc_row.custom_department_name, department, "ifffffffffffffff 222222")
	# 				continue

	# 			print(acc_row.account, "acc_row.account =======")

	# 			return acc_row.account
...
# --------------------------------------below is working function in case of ER + EE amounts-----------------------------
	def get_account(self, component_dict=None, employee=None):
		...
		for key in component_dict:
			...
			account = self.get_salary_component_accounts(component, fund, employee, department)
...
# --------------------------------------below is working function in case of ER + EE, ER expense separate amounts-----------------------------
	def get_employer_expense_account(self, component_dict=None, employee=None):
		...
		for key in component_dict:
			component, cost_center, fund, department, employee = key
			account = self.get_salary_component_account(component, fund, employee, department)
			account_map[key] = account
		...
		print(account_dict, "account_dict44444444 *************************")
